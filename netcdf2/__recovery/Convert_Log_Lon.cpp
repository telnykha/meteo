...
void ConLL(double Lot1, double Lon1, double Lot2, double Lon2, double NormX, double NormY, double Maxdist, double &xx, double &yy)
{

//Lot2, Lon2-center radar
  // a,b -picture size
 /* double gama1;
  double gama;
    ...
    else if (Lon1==Lon2 && Lot2>Lot1)
    {
     Angel=-90;
	  ...
	  if (Lot2<Lot1 && Lon2<Lon1)
      {
	   ...
	   //	double A=(Angel*180/PI_VALUE);
	 //xx=(cos(A*PI_VALUE/180))*dist;
	 //yy=(sin(A*PI_VALUE/180))*dist;
	 double Kx=NormX/(2*Maxdist);
	 double Ky=NormY/(2*Maxdist);
 // RCircel(dist,A, xx,yy);
   //	xx=(0.5*NormX-dist/450)*cos(Angel);
   //	yy=(0.5*NormY-dist/450)*sin(Angel);
	xx=0.5*NormX+Kx*dist*cos((Angel+180)*PI_VALUE/180);
	yy=0.5*NormY+Ky*dist*sin((Angel+180)*PI_VALUE/180);
  //xx = 0.5*NormX*(xx + 1);
  //yy = 0.5*NormY*(yy + 1);
}
