...
AWPRESULT _awpDrawThickPoint(awpImage* pImage, awpPoint p, AWPBYTE bChan, AWPDOUBLE dValue, AWPBYTE radius)
{
	...
	/* номер канала, на котором должна быть отрисована точка не может быть 
	   больше числа каналов входного изображени€ */
	if (bChan > pImage->bChannels)
		_ERROR_EXIT_RES_(AWP_BADARG)
	/*todo: значение €корсти рисуемой точки dValue не может выходить за допустимый диапазон, определ€емый
	выбранным типом изображени€*/
...
// перевод из пол€рных координат в декартовы исходной матрицы
...
ListView1->Clear();
	...
	int num = 0;
	double MI,MA,TETA; // полуоси всех €чеек
	awpPoint P; //цент масс всех €чеек
...
//		   VecList->Add(VP);
   ...
   //отрисока центра масс всех €чеек
		   awpRect r;
		   r.left = P.X-10;
		   r.right = P.X+10;
		   r.top = P.Y-10;
